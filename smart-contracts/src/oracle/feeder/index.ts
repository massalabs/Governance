...
async function main() {
    ...
    if (balanceMainnet < 1000n) {
        await alertsService.triggerAlert('balance-low', 'Balance of technical address is lower than 1000', 'critical');
    }
    ...
    log.info(`Mainnet Oracle Contract Address: ${contracts[NetworkName.Mainnet].oracle}`);
    log.info(`Buildnet Oracle Contract Address: ${contracts[NetworkName.Buildnet].oracle}`);
