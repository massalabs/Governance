...
function mockMasogBalance(balance: u64): void {
  mockScCall(u64ToBytes(balance));
}

function mockMasogTotalSupply(supply: u64): void {
...
function generateVote(proposalId: u64, value: i32, comment: string): Vote {
...
function setupProposal(
  ...
  proposal.status = stringToBytes(status);
  proposal.owner = stringToBytes(votingOwner);
  proposal.creationTimestamp = u64ToBytes(timestamp);
...
describe('Voting Contract', () => {
  ...
  describe('SubmitUpdateProposal', () => {
    ...
    test('submitUpdateProposal success', () => {
      ...
      expect(bytesToString(proposalStored.status)).toBe('DISCUSSION');
      expect(bytesToU64(proposalStored.creationTimestamp)).toBeGreaterThan(0);
  ...
  describe('Refresh', () => {
    ...
    test('Refresh transitions DISCUSSION to VOTING after 3 weeks', () => {
      ...
      const args = new Args().add<u64>(1).add<u32>(10).serialize();
      refresh(args);

      const proposalBytes = Storage.get(proposalKey(1));
      const proposal = new Proposal();
      proposal.deserialize(proposalBytes, 0);
      expect(bytesToString(proposal.status)).toBe(VOTING_STATUS);
    });
    ...
    test('Refresh transitions VOTING to ACCEPTED after 4 weeks with majority', () => {
      ...
      const proposal = new Proposal();
      proposal.deserialize(proposalBytes, 0);
      expect(bytesToString(proposal.status)).toBe(accepted);
    });
    ...
    test('Refresh transitions VOTING to REJECTED after 4 weeks without majority', () => {
      ...
      const proposal = new Proposal();
      proposal.deserialize(proposalBytes, 0);
      expect(bytesToString(proposal.status)).toBe(rejected);
    });
    ...
    test('Refresh does not change status if time not exceeded', () => {
      ...
      const proposal = new Proposal();
      proposal.deserialize(proposalBytes, 0);
      expect(bytesToString(proposal.status)).toBe(discussion);
    });

    throws('Refresh fails if startId exceeds total proposals', () => {
      const args = new Args().add<u64>(2).add<u32>(10).serialize(); // No proposals yet
      refresh(args);
    });
  });
});
