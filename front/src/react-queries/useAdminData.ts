...
const fetchAdminData = async (): Promise<AdminData> => {
  const { governancePublic, masOgPublic, oraclePublic } =
  ...
  try {
    ...
    const [balances, govEvents, currentSlot] = await Promise.all([
    ...
    const currentPeriod = BigInt(currentSlot?.lastSlot?.period ?? 0);
